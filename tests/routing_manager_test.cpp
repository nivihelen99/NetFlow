...
protected:
    ...
    void SetUp() override {
        rm = RoutingManager(); // Ensure clean state for each test

        network1_addr = IpAddress("192.168.1.0");
        network1_mask = IpAddress("255.255.255.0"); // /24
        network1_next_hop = IpAddress("10.0.0.1");
        network1_if_id = 1;

        network2_addr = IpAddress("192.168.1.128");
        network2_mask = IpAddress("255.255.255.128"); // /25
        network2_next_hop = IpAddress("10.0.0.2");
        network2_if_id = 2;

        default_route_addr = IpAddress("0.0.0.0");
        default_route_mask = IpAddress("0.0.0.0"); // /0
        default_route_next_hop = IpAddress("10.0.0.3");
...
TEST_F(RoutingManagerTest, AddAndLookupRoute) {
    ...
    auto route = rm.lookup_route(IpAddress("192.168.1.10"));
...
TEST_F(RoutingManagerTest, LookupNonExistentRoute) {
    auto route = rm.lookup_route(IpAddress("172.16.0.1"));
...
TEST_F(RoutingManagerTest, AddMultipleRoutesAndLookup) {
    ...
    auto route1 = rm.lookup_route(IpAddress("192.168.1.10"));
    ...
    auto route_default = rm.lookup_route(IpAddress("100.100.100.1"));
...
TEST_F(RoutingManagerTest, LongestPrefixMatch) {
    ...
    auto route_specific = rm.lookup_route(IpAddress("192.168.1.130"));
    ...
    auto route_general = rm.lookup_route(IpAddress("192.168.1.10"));
...
TEST_F(RoutingManagerTest, RemoveRoute) {
    rm.add_static_route(network1_addr, network1_mask, network1_next_hop, network1_if_id);
    auto route_before_remove = rm.lookup_route(IpAddress("192.168.1.10"));
    ASSERT_TRUE(route_before_remove.has_value());

    rm.remove_static_route(network1_addr, network1_mask);
    auto route_after_remove = rm.lookup_route(IpAddress("192.168.1.10"));
    EXPECT_FALSE(route_after_remove.has_value());
}

TEST_F(RoutingManagerTest, AddRouteWithZeroNextHop) {
    IpAddress directly_connected_network("192.168.5.0");
    IpAddress directly_connected_mask("255.255.255.0");
    IpAddress zero_next_hop("0.0.0.0");
    uint32_t if_id = 5;

    rm.add_static_route(directly_connected_network, directly_connected_mask, zero_next_hop, if_id);
    auto route = rm.lookup_route(IpAddress("192.168.5.50"));
    ASSERT_TRUE(route.has_value());
...
TEST_F(RoutingManagerTest, RemoveNonExistentRoute) {
    ...
    rm.remove_static_route(IpAddress("10.10.10.0"), IpAddress("255.255.255.0"));
...
TEST_F(RoutingManagerTest, AddDuplicateRoute) {
    ...
    rm.add_static_route(network1_addr, network1_mask, IpAddress("10.0.0.100"), network1_if_id + 1, 5);

    auto table = rm.get_routing_table();
    ASSERT_EQ(table.size(), 1);
    EXPECT_EQ(table[0].next_hop_ip, IpAddress("10.0.0.100"));
...
TEST_F(RoutingManagerTest, RouteLookupOrderWithSamePrefixLength) {
    ...
    IpAddress routeA_addr("192.168.3.0");
    IpAddress routeA_mask("255.255.255.0"); // /24
    IpAddress routeA_next_hop("10.0.3.1");
    uint32_t routeA_if_id = 13;

    IpAddress routeB_addr("192.168.3.0"); // Same dest/mask as A
    IpAddress routeB_mask("255.255.255.0"); // /24
    IpAddress routeB_next_hop("10.0.3.2");
    ...
    IpAddress routeC_addr("192.168.4.0");
    IpAddress routeC_mask("255.255.255.0"); // /24
    IpAddress routeC_next_hop("10.0.4.1");
    ...
    auto lookup = rm.lookup_route(IpAddress("192.168.3.100"));
